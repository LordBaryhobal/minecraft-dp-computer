execute if score debug_instructions cbc_settings matches 1 run tellraw @a "OUT R2"

data modify storage cb_computer:storage output set from storage cb_computer:storage Registers[1]
execute if score output_chat cbc_settings matches 1 run tellraw @a {"storage":"cb_computer:storage","nbt":"Registers[1]"}
